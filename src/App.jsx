...
function App() {
	...
	const columnHeaders = [...Array.from({ length: 15 }, (_, i) => `PO${i + 1}`)];
	const POPSOmap = {
		PO13: "PSO1",
		PO14: "PSO2",
		PO15: "PSO3",
	...
	const handleFileChange = async (e) => {
		...
		if (["xlsx", "xls", "csv"].includes(ext)) {
			try {
				...
				setMappingError(
					"Failed to parse file. Please upload a valid Excel or CSV file with mapping values (0-3)."
				);
			}
		}
	};
	...
	const handleSubmit = async () => {
		...
		try {
			const response = await axios.post(
				"https://copo-server-3-0.onrender.com/direct-process",
				formData,
				{
					headers: {
							...
							<div style={{ height: "auto", width: "100%" }}>
								...
								<DataGrid
									...
									processRowUpdate={(newRow, oldRow) => {
										...
										if (invalid) {
											setMappingError(
												"Please enter mapping values between 0-3"
											);
							...
							<Box mt={2}>
								<Paper
									...
									sx={{
										p: 3,
										background:
											"linear-gradient(90deg, #e3f2fd 0%, #fce4ec 100%)",
										borderRadius: 3,
										mb: 2,
									}}
								>
									...
									<Typography
										...
										sx={{ mb: 2 }}
									>
										Please use the following format for your upload. Enter marks
										for each test in the columns below.
									</Typography>
									<TableContainer
										component={Paper}
										sx={{
										...
										<Table size="medium">
											<TableHead>
												<TableRow
													sx={{
														background:
															"linear-gradient(90deg, #bbdefb 0%, #f8bbd0 100%)",
													}}
												>
													<TableCell
														align="center"
							...
							<ResponsiveContainer width="100%" height={350}>
								<BarChart
									data={columnHeaders.map((header) => ({
										name: POPSOmap[header] || header,
										value: Number(
											((output.po_values[header] || 0) * 0.8).toFixed(2)
										),
									}))}
						...
						{showIndirect ? (
							<Indirect
								...
								directPO80={
									output && output.po_values
										? Object.fromEntries(
												Object.entries(output.po_values).map(([key, value]) => [
													key,
													value * 0.8,
												])
										  )
